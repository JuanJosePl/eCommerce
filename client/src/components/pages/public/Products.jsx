...
const ProductListingPage = () => {
  const {
    data: productsData = { productos: [], currentPage: 1, totalPages: 1, totalItems: 1 },
    error,
    loading,
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) =>
      product.nombre.toLowerCase().includes(searchTerm.toLowerCase()) // Cambia `product_name` a `nombre`
    );
  }, [products, searchTerm]);
